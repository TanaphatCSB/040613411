...
/*1-5
1.SELECT std_id, std_name, province FROM student ORDER BY std_name ASC;
2.SELECT std_id, std_name FROM student;
3.SELECT std_id, std_name, province FROM student WHERE province = 'ขอนแก่น';
4.SELECT c.course_id, c.title, c.credit FROM course c 
INNER JOIN register r ON c.course_id = r.course_id
WHERE r.std_id = '5001100348';
5. SELECT std_id, SUM(credit) AS total_credits
FROM register
